...
terraform init -backend-config="storage_account_name=$TERRAFORM_STORAGE_NAME" -backend-config="resource_group_name=$TERRAFORM_STATE_RESOURCE_GROUP_NAME"  -backend-config="container_name=tfstate" -backend-config="access_key=$TERRAFORM_STORAGE_KEY" -backend-config="key=codelab.microsoft.tfstate" ./environment

echo "planning terraform..."
terraform plan -out $deployment_name-out.plan -var-file "config/$deployment_name.tfvars"  -var="deployment_name=$deployment_name" -var="location=$location" -var="tenant_id=$tenantid" -var="subscription_id=$subscriptionid"  ./environment
